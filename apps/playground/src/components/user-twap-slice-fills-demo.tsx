...
export function UserTwapSliceFillsDemo() {
  ...
  const { data, loading, error, ready } = useUserTwapSliceFills(address ?? DEFAULT_ADDRESS, {
  ...
  const fills = useMemo(() => {
    return [...(data?.twapSliceFills ?? [])]
      .map((entry) => {
        ...
        const displayCoin = symbolConverter?.getSpotByPairId(fill.coin) ?? fill.coin;
    ...
    <section className="space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-sm text-gray-500">
          Table demo for <code>useUserTwapSliceFills</code>, merging snapshot and incremental TWAP
          slice fill events for a wallet.
        </p>
      </div>
      ...
      <Card className={`${DEMO_CARD_CLASS_NAME} text-[#183242]`}>
        <CardHeader className={DEMO_CARD_HEADER_CLASS_NAME}>
          ...
          >
            ...
            <span>{ready ? `${fills.length} fills` : loading ? "Loading..." : "Idle"}</span>
          </div>
        </CardHeader>
              ...
              <table className="min-w-[1580px] table-auto border-separate border-spacing-0">
                ...
                <tbody>
                  {fills.map((fill, index) => (
                    ...
                    >
                      <td className="px-4 py-3 text-[#183242]">{formatDate(fill.time)}</td>
                      <td className="px-4 py-3 text-[#183242]">{fill.twapId}</td>
                      <td className="px-4 py-3 text-[#183242]">
                        <div className="font-semibold">{fill.coin}</div>
                        {fill.coin !== fill.rawCoin ? (
                          <div className="text-[11px] text-[#6f8797]">{fill.rawCoin}</div>
                        ) : null}
                      </td>
                      <td className={`px-4 py-3 ${getSideClass(fill.side)}`}>
                        {getSideLabel(fill.side)}
                      </td>
                      <td className="px-4 py-3 text-[#183242]">${formatAmount(fill.px)}</td>
                      <td className="px-4 py-3 text-[#183242]">{formatAmount(fill.sz)}</td>
                      <td className="px-4 py-3 text-[#183242]">
                        {formatAmount(fill.startPosition)}
                      </td>
                      <td className="px-4 py-3 text-[#183242]">${formatAmount(fill.closedPnl)}</td>
                      <td className="px-4 py-3 text-[#183242]">
                        {formatAmount(fill.fee)} {fill.feeToken}
                      </td>
                      <td className="px-4 py-3 text-[#183242]">{fill.crossed ? "Yes" : "No"}</td>
                      <td className="px-4 py-3 text-[#183242]">{truncateHash(fill.hash)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
