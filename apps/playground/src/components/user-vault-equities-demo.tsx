...
export function UserVaultEquitiesDemo() {
  ...
  const { data, isPending, error, isFetched } = useUserVaultEquities(address ?? DEFAULT_ADDRESS, {
    enabled: Boolean(address),
  });

  const summary = useMemo(() => {
    const entries = data ?? [];
    const totalEquity = entries.reduce((sum, item) => sum + Number(item.equity), 0);
    ...
    <section className="space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-sm text-gray-500">
          Query demo for <code>useUserVaultEquities</code>, showing vault deposits for a tracked
          wallet.
        </p>
      </div>
      ...
      <Card className={`${DEMO_CARD_CLASS_NAME} text-[#183242]`}>
        <CardHeader className={DEMO_CARD_HEADER_CLASS_NAME}>
          ...
          <div className={`${DEMO_CARD_STATUS_CLASS_NAME} flex items-center justify-between gap-4`}>
            ...
            <span>
              {isPending ? "Loading..." : isFetched ? `${summary.count} vaults` : "Idle"}
            </span>
          </div>
        </CardHeader>
            ...
            <>
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-[24px] border border-stone-200 bg-stone-50/80 p-4">
                  ...
                  <div className="mt-3 text-2xl font-semibold text-stone-900">{summary.count}</div>
                </div>
                <div className="rounded-[24px] border border-stone-200 bg-stone-50/80 p-4">
                  <div className="text-[11px] font-medium uppercase tracking-[0.18em] text-stone-400">
                    Total Equity
                  ...
                  {data?.map((item) => (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="truncate font-medium text-stone-900">{item.vaultAddress}</div>
                        <div className="text-xs text-stone-500">Vault address</div>
                      </div>
