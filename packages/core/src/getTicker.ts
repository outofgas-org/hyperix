...
export async function getTickerContext(options: GetHyperliquidTickerOptions): Promise<{
    ...
    if (options.marketType === "perp") {
        ...
        if (index === -1 || !assetCtxs[index]) {
            throw new HyperliquidCoreError(`Unable to resolve perpetual market price for ${options.coin}`);
        }
    ...
    if (!pairId) {
        throw new HyperliquidCoreError(`Unable to resolve spot market metadata for ${options.coin}`);
    }
    ...
    if (!context) {
        throw new HyperliquidCoreError(`Unable to resolve spot market price for ${options.coin}`);
    }
...
function requireBook<T extends NonNullable<unknown>>(orderbook: T | null, coin: string): T {
    if (!orderbook) {
...
export async function getTicker(options: GetHyperliquidTickerOptions): Promise<HyperliquidTicker> {
    const infoClient = getDefaultInfoClient();
    const {normalizedCoin, context} = await getTickerContext(options);
    const book = requireBook(await infoClient.l2Book({coin: normalizedCoin}), options.coin);
