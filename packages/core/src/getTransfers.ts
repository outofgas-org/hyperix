...
import {
  type BuildHyperliquidSendAssetOptions,
  type BuilderHyperliquidInternalTransferOptions,
  HyperliquidCoreError,
  type HyperliquidInternalTransfer,
  type HyperliquidSendAsset,
} from "./types";
...
export async function buildInternalTransfer(
  ...
  const perpMeta = allPerpMetas.find(
    (dexMetadata) =>
      getCoinDexName(dexMetadata.universe[0]?.name ?? "") === normalizedDexName,
  );

  if (!perpMeta) {
    throw new HyperliquidCoreError(
      `Unable to resolve perpetual dex for ${normalizedDexName}`,
    );
  }
  ...
  if (!collateralToken?.tokenId) {
    throw new HyperliquidCoreError(
      `Unable to resolve transfer token for ${normalizedDexName}`,
    );
  }
...
export async function buildSendAsset(
  ...
  if (normalizedAsset.includes("/")) {
    throw new HyperliquidCoreError(
      "Spot send asset must be a base asset symbol, not a trading pair",
    );
  }
...
export async function resolveSpotTransferToken(asset: string): Promise<string> {
  ...
  const token = spotMeta.tokens.find(
    (item) => item.name.toUpperCase() === asset,
  );
