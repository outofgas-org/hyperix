import {DefaultHyperliquidMetadataCache, type HyperliquidMetadataCache} from "./lib/metadataInfo";
import type {
    AllPerpMetas,
...
export function getDexNamesFromAllPerpMetas(allPerpMetas: AllPerpMetas): string[] {
...
export function requireByUppercase<T extends { coin: string }>(items: T[], coin?: string): T[] {
    if (!coin) {
        return items;
    }
